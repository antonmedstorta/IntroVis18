	...
	var yForce = d3.forceY(height / 2).strength(0.1)
...
// FOLLOWING A USER BUTTON CLICK
	...
	d3.select("#ComSkills").on("click", function() {
		var buttons = (document.getElementsByClassName("btn btn-primary"));
		for (i = 0; i < buttons.length; i++) { 
    		buttons[i].style.backgroundColor = "#007bff";
		}
		document.getElementById("ComSkills").style.backgroundColor = "#1cd760";
		simulation
			.force("collision", d3.forceCollide(function(d) {
				return radiusScale(d.ComSkills);
			}))
			.alphaTarget(0.04)
			.restart() //re-add force upon clicking a button
		circles
